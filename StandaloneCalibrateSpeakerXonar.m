function varargout = StandaloneCalibrateSpeakerXonar(varargin)

%Help for StandaloneCalibrateSpeakerXonar
%Xonar version. Tweaked to work with a Xonar soundcard, on linux.
%for some reason the sound in doesn't seem to be supported, so we are using
%indpendent PPA objects for input (using built-in soundcard on motherboard)
%and for output (using Xonar). Note that Lynx (which isn't supported in
%linux) on windows works in full duplex mode, which is the other version of
%StandaloneCalibrateSpeaker
%
%
% Description:
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @StandaloneCalibrateSpeakerXonar_OpeningFcn, ...
    'gui_OutputFcn',  @StandaloneCalibrateSpeakerXonar_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before StandaloneCalibrateSpeakerXonar is made visible.
function StandaloneCalibrateSpeakerXonar_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to StandaloneCalibrateSpeakerXonar (see VARARGIN)

% Choose default command line output for StandaloneCalibrateSpeakerXonar
...
% so window can get raised using StandaloneCalibrateSpeakerXonar.
...
% UIWAIT makes StandaloneCalibrateSpeakerXonar wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = StandaloneCalibrateSpeakerXonar_OutputFcn(hObject, eventdata, handles)
...
for lp = 1:num_loops
    ...
    for i=1:length(logspacedfreqs) %play tones
        ...
        %fudge factor for built-in soundcard input device (using Xonar output device)
        fudgefactorTone=-2.07;
        fudgefactorWN=7;
        ylabel(ax2, 'PSD');
        if tonefreq==-1
...
OutputDeviceID=GetXonarDevice; %userdata.DeviceID;
...
try paOuthandle = PsychPortAudio('Open', OutputDeviceID, outputMode, reqlatencyclass, SoundFs, numChan, buffSize);
 %hack mw 10.13.2016:
 paInhandle = PsychPortAudio('Open', InputDeviceID, inputMode, reqlatencyclass, SoundFs, numChan, buffSize);
...
DeviceID=GetXonarDevice;
set(hObject, 'Value',DeviceID+1); %default Output Device ID, note devs is 0-indexed
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
help StandaloneCalibrateSpeakerXonar
