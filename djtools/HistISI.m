...
if nargout == 0 || ~isempty(axesHandle)  % no output args, plot it
    ...
    if sum(ISI<0.002)>0
        text(min(get(axesHandle,'xLim')), max(get(axesHandle, 'yLim')), ...
            ...
            'VerticalAlignment','top','HorizontalAlignment','left','color',myColor);
        str = num2str(round((sum(ISI<0.002)/length(T))*100,2));
    else
        str = '0';
    end        
    set(gca, 'YTick', max(H));   
    yl = ylim;
    text(.1,range(yl)*.95,[str '% of ' num2str(length(T)) ' total spikes'],'fontsize',12,'color',myColor) 
end
