function [filename,path]=MakeF1F2GapProtocol(F1, F2, amplitude, ...
...
% F1=4000; F2=10000; amplitude=70; include_whitenoise=1; duration=500; ramp=3; iti=1000; nrepeats=20;
% gapdurs=[16 128]; include_silent_sound=0;
% MakeF1F2GapProtocol(F1, F2, amplitude, include_whitenoise, duration, ramp, iti, nrepeats, gapdurs, include_silent_sound)
...
name= sprintf('F1F2Gap %s/%d & %d Hz/%d dB/tone dur %d ms/%s ms gaps/%s/%d ms iti/%d reps', ...
    include_whitenoisestr, F1,F2, amplitude,duration, gapdursstring, ...
    include_silent_soundstr, iti,nrepeats);
description= sprintf('F1F2Gap %s/%d & %d Hz/%d dB/tone dur %d ms/%s ms gaps/%s/%d ms iti/%d reps ,%d stim per rep, %d total stimuli, %ds per rep, %d s total dur', ...
    include_whitenoisestr, F1,F2, amplitude,duration, gapdursstring, ...
    include_silent_soundstr, iti,nrepeats, ...
    StimPerRepeat,TotalNumStim, round(DurationPerRepeatSecs), round(TotalDurationSecs));

filename= sprintf('F1F2Gap-%s-%d-%dHz%ddB-tonedur%dms-%s-msgaps-%s-%dmsiti-%dreps', ...
    include_whitenoisestr, F1,F2, amplitude,duration, gapdursstring, ...
    include_silent_soundstr, iti,nrepeats);
