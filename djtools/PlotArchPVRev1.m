...
for clustindex=1:length(outfilename) %main cluster loop
    ...
    %plot the mean tuning curve OFF, only a single panel
    for windex=1:numw
        figure
        p=0;
...
%         end
      ...
      %plot the mean tuning curve OFF, with a lot of blank panels
    for windex=1:numw
        figure
        p=0;
        subplot1(numy,numfreqs, 'Max', [.95 .9])
    ...
    if IL
                    ...
                    if rasters==1
                        for n=1:nrepsON(findex, aindex, dindex)
                            ...
                            for np=1:MLaserNumPulses
                                plot([MLaserStart+(np-1)*(MLaserWidth+MLaserISI) MLaserStart+(np-1)*(MLaserWidth+MLaserISI)+MLaserWidth], [1 1]+yl(2)+offset, 'g')
                            end
                            h=plot(spiketimes2, yl(2)+ones(size(spiketimes2))+offset, '.k');
                        end
                    end
      ...
      %plot the mean tuning curve ON again, split into 2 columns for
      %readability
        for windex=1:numw
            figure
            p=0;
            subplot1(ceil(numy/2),2, 'Max', [.95 .9], 'Gap', [.01 0])
            for yindex=ystart:ystep:yend
                for findex=1:numfreqs
                    p=p+1;
                    if p<=numy/2
