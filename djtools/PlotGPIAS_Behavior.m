function PlotGPIAS_Behavior(datadir)
...
%                 add the stimulus in magenta
                        stimtrace=squeeze(mM1OFF(gdindex, paindex,:));
            ...
            subplot1(1)
            h=title(sprintf('%s:\nnreps: %d-%d, OFF',datadir,min(nrepsOFF(:)),max(nrepsOFF(:))));
        ...
        if ~isempty (mM1ON)
            ...
            for gdindex=1:numgapdurs
                ...
                trace1=trace1-median(trace1(1:1000));
%                 stim=squeeze(mM1ONstim(gdindex, paindex,:));
