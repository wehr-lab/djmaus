function PlotGPIASflashtrain_PSTH(varargin)

%plots clustered spiking GPIAS data from djmaus
%
% usage: PlotGPIASflashtrain_PSTH([datapath], [tetrode], [clust], [xlimits],[ylimits], [binwidth])
...
if isempty(channel)     %default to all tetrodes
    ...
    for i=1:length(d)
        fn=d(i).name;
        PlotGPIASflashtrain_PSTH_single(datadir, fn, xlimits, ylimits, binwidth)
    end
    if isempty(d)
