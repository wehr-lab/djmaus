function PlotSpeechContextMaster(varargin)
...
if isempty(channel)     %default to all tetrodes
    ...
    if ~isempty(d)
        for i=1:length(d)
            PlotSpeechContext_singleMaster(datadir, d(i).name, xlimits, ylimits, binwidth)
%             close all %close each new PSTH as they are generated for directories with many cells/outfiles SFM 4/9/21
    ...
    elseif exist('dirs.mat','file') %Nick addition 8/31/18 - defaults to kilosort output if present. Otherwise defaults to .t files.
        ...
        save('masterdir.m', 'masterdir');
        poolobj = gcp;
        addAttachedFiles(poolobj, 'masterdir.m');
        parfor i=1:length(sp.cids) %each iteration here therefore corresponds to the 'cellnum' used in readKiloSortOutput.
            ...
            PlotSpeechContext_singleMaster(datadir, fn, xlimits, ylimits, binwidth)
            close all %close each new PSTH as they are generated for directories with many cells/outfiles SFM 4/9/21
        end
    ...
    if isempty(clust) % default to all clusters
        ...
        for i=1:length(d)
            fn=d(i).name;
            PlotSpeechContext_singleMaster(datadir, fn, xlimits, ylimits)
            close all 
        end
    else 
        if clust<10
