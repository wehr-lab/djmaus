function PlotSustainedSuppression_single(varargin)
...
%
% usage: PlotSustainedSuppression_single(datapath, t_filename, [xlimits],[ylimits], [binwidth])
    ...
    for tnpindex=1:numtrainnumpulses
        for tpwindex=1:numtrainpulsewidths
            for tiindex=1:numtrainisis
                ...
                X=xlimits(1):binwidth:xlimits(2); %specify bin centers
                [N, x]=hist(st, X);
                N=N./nrepsTrain; %normalize to spike rate (averaged across trials)
                N=1000*N./binwidth; %normalize to spike rate in Hz
                ymax= max(ymax,max(N));
            end
        end
