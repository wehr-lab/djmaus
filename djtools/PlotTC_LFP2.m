...
if isfield(out, 'channelmap_file')
    bad_channels_from_xml=ReadImpedanceTestXML(out.channelmap_file); %reads impedance test xml file generated by open ephys, prints and plots results
else
    %out.channelmap_file not saved (e.g. non-128ch probe, or map failed to load during processing) -- fall back to ReadImpedanceTestXML's own location guesses
    bad_channels_from_xml=ReadImpedanceTestXML;
end
try
