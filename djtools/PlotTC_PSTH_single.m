...
try
    ...
    fprintf('\n Did not find Silent Sound stimuli\n')
end
fs=10; %fontsize
...
for windex=1:numw
    ...
    for yindex=ystart:ystep:yend
        for findex=1:numfreqs
            ...
            N=1000*N./binwidth; %normalize to spike rate in Hz
            if ~isempty(mSilentSoundOFF.spiketimes) && aindex==1 && isempty(spiketimes1)
                spiketimes1=mSilentSoundOFF.spiketimes;
                X=xlimits(1):binwidth:xlimits(2); %specify bin centers
                [N, x]=hist(spiketimes1, X);
                N=N./length(SilentSoundOFF); %normalize to spike rate (averaged across trials)
                N=1000*N./binwidth; %normalize to spike rate in Hz
            end
            offset=ylimits(2);
            yl=ylimits;
    ...
    for windex=1:numw
        ...
        for yindex=ystart:ystep:yend
            for findex=1:numfreqs
                ...
                N=1000*N./binwidth; %normalize to spike rate in Hz
                if ~isempty(mSilentSoundON.spiketimes) && aindex==1 && isempty(spiketimes1)
                    spiketimes1=mSilentSoundON.spiketimes;
                    X=xlimits(1):binwidth:xlimits(2); %specify bin centers
                    [N, x]=hist(spiketimes1, X);
                    N=N./length(SilentSoundON); %normalize to spike rate (averaged across trials)
                    N=1000*N./binwidth; %normalize to spike rate in Hz
                end
                offset=0;
                yl=ylimits;
...
if IL && plotONOFF
    for windex=1:numw
        ...
        for yindex=ystart:ystep:yend
            for findex=1:numfreqs
                ...
                spiketimesON=mM1ON(findex, aindex, dindex).spiketimes;
                spiketimesOFF=mM1OFF(findex, aindex, dindex).spiketimes;
                X=xlimits(1):binwidth:xlimits(2); %specify bin centers
                [NON, xON]=hist(spiketimesON, X);
                [NOFF, xOFF]=hist(spiketimesOFF, X);
                NON=NON./nrepsON(findex, aindex, dindex);
                NOFF=NOFF./nrepsOFF(findex, aindex, dindex);%normalize to spike rate (averaged across trials)
                NON=1000*NON./binwidth; %normalize to spike rate in Hz
                NOFF=1000*NOFF./binwidth;
                offset=ylimits(2);
                yl=ylimits;
                inc=(yl(2))/max(max(max(nreps)));
                if ~isempty(mSilentSoundON.spiketimes) && aindex==1 && isempty(spiketimesON)
                    spiketimes1=mSilentSoundON.spiketimes;
                    X=xlimits(1):binwidth:xlimits(2); %specify bin centers
                    [NON, xON]=hist(spiketimes1, X);
                    NON=NON./length(SilentSoundON); %normalize to spike rate (averaged across trials)
                    NON=1000*NON./binwidth; %normalize to spike rate in Hz
                end
                if ~isempty(mSilentSoundOFF.spiketimes) && aindex==1 && isempty(spiketimesOFF)
                    spiketimes1=mSilentSoundOFF.spiketimes;
                    X=xlimits(1):binwidth:xlimits(2); %specify bin centers
                    [NOFF, xOFF]=hist(spiketimes1, X);
                    NOFF=NOFF./length(SilentSoundOFF); %normalize to spike rate (averaged across trials)
            ...
            if StimRecorded
                ...
                t=1000*t/out.samprate; %convert to ms
                t=t+out.xlimits(1); %correct for xlim in original processing call
                offset=ylimits(1)+.5*diff(ylimits);
                stimh=plot(t, Stimtrace+offsetS, 'm');
                uistack(stimh, 'top')
                ylimits2(1)=0;%2*offsetS;
            else
    ...
    subplot1(1)
    h=title(sprintf('%s: \ntetrode%d cell%d %dms, nreps: %d-%d, ON and OFF',datadir,channel,out.cluster,durs(dindex),min(min(min(nrepsON))),max(max(max(nrepsON)))));
    set(h, 'HorizontalAlignment', 'center', 'interpreter', 'none', 'fontsize', fs, 'fontw', 'normal')
