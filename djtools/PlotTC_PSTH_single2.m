function PlotTC_PSTH_single2(varargin)

%plots a single file of clustered spiking tuning curve data from djmaus from outfilename
%
% usage: PlotTC_PSTH_single2(datapath, outfilename, [xlimits],[ylimits], [binwidth])
...
if exist(outfilename,'file')
    ...
    fprintf('\nloaded outfile.')
       ...
       error('')
       ProcessTC_PSTH([], out.SortedUnits, out.BonsaiPath, out.EphysPath, out.EphysPath_KS, xlimits,ylimits)
    ProcessTC_PSTH_single(datadir,  t_filename, xlimits, ylimits);
    load(outfilename);
end
...
if ~isempty(xlimits)
    if out.xlimits(1)>xlimits(1) | out.xlimits(2)<xlimits(2) %xlimits in outfile are too narrow, so reprocess
        ...
        ProcessTC_PSTH_single2(out.cell, out.SortedUnits, out.BonsaiPath, out.EphysPath, out.EphysPath_KS, xlimits,ylimits);
        load(outfilename);
    end
end
...
for windex=1:1                                                              % Normally windex=1:numw, hardcoded here for efficiency
    ...
    for yindex=ystart:ystep:yend
        for findex=1:numfreqs
            ...
            %set(gca, 'yticklabel', '')
            if p==1
                h=title(sprintf('%s: \ncell%d %dms, nreps: %d-%d, OFF ',datadir,out.cell,durs(dindex),min(min(min(nrepsOFF))),max(max(max(nrepsOFF)))));
