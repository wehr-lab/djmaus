...
if ~strcmp(GetPlottingFunction(datadir), 'PlotAsymGPIAS_PSTH')
    error('This does not appear to be a AsymGPIAS stimulus protcol');
end
...
% M1ON(numgapdurs, numonramps, numofframps, nrepsON).spiketimes
% mM1ON(numgapdurs, numonramps, numofframps).spiketimes
% mM1ONspikecount(numgapdurs, numonramps, numofframps)
...
if IL
    ...
    out.LaserStart=unique(LaserStart);
