function ProcessGPIAS_BehaviorMouse2(out)

% modified from ProcessGPIAS_BehaviorTilt,
% this is called from ProcessGPIAS_BehaviorTilt to plot behavior from a
% second simultaneously recorded mouse
%
%processes  behavioral data from djmaus
%
% usage: ProcessGPIAS_BehaviorMouse2(out) 
...
% %         fprintf('\nnotebook file has a second mouse ID, will try to process that')
% %     end
% % end
% % 
% % %read messages
...
if exist(filename5, 'file')~=2 %couldn't find it
    error(sprintf('could not find ADC5 file %s in datadir %s', filename4, datadir))
    %this should be impossible since we checked for existence of the file prior to invoking this function 
end
[scaledtrace, datatimestamps, datainfo] =load_open_ephys_data(filename5);
scaledtrace = scaledtrace - mean(scaledtrace);
fprintf('\nUsing TILT PLATFORM\n')
