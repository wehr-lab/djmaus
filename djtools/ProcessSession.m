...
keys=session.recordNodes{1}.recordings{1}.continuous.keys();
    ...
    elseif num_channels==43 %32-ch tetrode config
        ...
        lasertracech=38;
    elseif num_channels==11 %Rig2 gap detection on at least one behavior config
        stimtracech=4;
    ...
    elseif num_channels==27 %Rig2 gap detection on a different behavior config?
        stimtracech=20;
...
%it would be better to get the correct ttl map key from the info
