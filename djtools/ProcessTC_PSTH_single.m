...
ta for all cells clustered for that tetrode
% saves to outfile
...
out.LaserStart=unique(LaserStart); %only saving one value for now, assuming it's constant
out.LaserWidth=unique(LaserWidth);
out.LaserNumPulses=unique(LaserNumPulses);function ProcessTC_PSTH_single(varargin)
...
out.nrepsON=nrepsON;
out.nrepsOFF=nrepsOFF;
out.xlimits=xlimits;function ProcessTC_PSTH_single(varargin)
