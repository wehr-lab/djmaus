...
%-mike 04.28.2024
%
% usage: ProcessTC_PSTH([cells], SortedUnits, BonsaiPath, EphysPath, EphysPath_KS, [xlimits],[ylimits])
% [cells] is the list of units to plot, send empty [] to process all cells, use indexes of SortedUnits (i.e. an integer from 1:numSortedUnits
...
cells=varargin{1};
SortedUnits=varargin{2};
BonsaiPath=varargin{3};
EphysPath=varargin{4};
EphysPath_KS=varargin{5};

if isempty(cells)
    cells=1:length(SortedUnits);
end
...
for cellnum=[cells]
    ...
    outfilename=sprintf('outPSTH_cell%d.mat',cellnum);
    save (outfilename, 'out')

end %for cellnum=[cells]