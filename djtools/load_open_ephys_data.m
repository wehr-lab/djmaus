...
MAX_NUMBER_OF_CONTINUOUS_SAMPLES = 1e6;
...
elseif strcmp(filetype, 'continuous')
    ...
    %9-0-2016 the problem, as I see it, is that info.ts entries are
    %occasionally less than 1024 apart. but nsamples is always 1024. WHen
    %this happens, ts_interp is extended beyond the next info.ts record.
    %And thus timestamps backtracks (sawtooths) since it will then start at
    %the next info.ts record, which is earlier. Why is info.ts getting
    %timstamps less than nsamples apart???
