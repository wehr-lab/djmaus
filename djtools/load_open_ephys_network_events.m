function [data, timestamps, info] = load_open_ephys_network_events(filename)

%modified from load_open_ephys_data to work with network events messages,
%which were not handled by the original load_open_ephys_data
%mw 8.23.2016

% [data, timestamps, info] = load_open_ephys_data(filename)
%
%   Loads messages.events files into Matlab.
%
%   Inputs:
%
...
SPIKE_PREALLOC_INTERVAL = 1e3;%used to be 1e6
...
%     
%      if filesize<= 1024
%          fprintf('\nNo data in one of these tetrode files, I will now crash. Try again without the tetrode that only has a 1kb filesize')
%      end
